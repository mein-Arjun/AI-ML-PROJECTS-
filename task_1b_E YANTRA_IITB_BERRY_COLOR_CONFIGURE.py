...
def get_vision_sensor_image(client_id):
	...
	##############	ADD YOUR CODE HERE	##############
	return_code,objectHandle=sim.simxGetObjectHandle(client_id,'vision_sensor',sim.simx_opmode_blocking)
	return_code,image_resolution,vision_sensor_image=sim.simxGetVisionSensorImage(client_id,objectHandle,0,sim.simx_opmode_streaming)
...
def detect_qr_codes(transformed_image):
	...
	for barcode in decode(transformed_image):
		...
		cx=(x+w)/2
		cy=(y+h)/2
		qr_codes.append([BarcodeData,(cx,cy)])    
